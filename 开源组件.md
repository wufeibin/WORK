...
| Highgui  | OpenCV 3.0中分为 imcodecs、videoio、highgui 三部分。包含可以用来显示图像或者简单的输入的用户交互函数，可以看作是一个轻量级的  Windows UI 工具包。 | libopencv_imgcodecs.so<br />libopencv_videoio.so<br />libopencv_highgui.so |
