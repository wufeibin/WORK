> 数据结构就是一组数据的存储结构，算法就是操作一组数据的方法。数据结构是为算法服务的，算法要作用在特定的数据结构之上。
...
堆其实就是一棵完全二叉树，每个父节点的值大/小于等于子节点的值。
...
邻接矩阵的优点是查找快捷，如果给定一个 `edge(i,j)` 的边，则可直接在矩阵中通过 `graph[i][j]` 查询。直观且表现力强，**无向图**中为 1 的数字总是对称，而对于**无向权重图**则是非 0 的数字总是对称。
...
<img src="https://labuladong.gitee.io/algo/images/%e4%ba%8c%e5%88%86%e5%ba%94%e7%94%a8/title1.png" alt="img" style="zoom: 50%;" />
...
<img src="https://labuladong.gitee.io/algo/images/%e4%ba%8c%e5%88%86%e5%ba%94%e7%94%a8/title2.png" alt="img" style="zoom: 50%;" />
...
<img src="https://labuladong.gitee.io/algo/images/slidingwindow/title1.png" alt="img" style="zoom: 50%;" />

初始状态：

<img src="https://labuladong.gitee.io/algo/images/slidingwindow/1.png" alt="img" style="zoom: 33%;" />

增加 `right`，直到窗口 `[left, right]` 包含了 `T` 中所有字符。

<img src="https://labuladong.gitee.io/algo/images/slidingwindow/2.png" alt="img" style="zoom: 33%;" />

现在开始增加 `left`，缩小窗口 `[left, right]`。

<img src="https://labuladong.gitee.io/algo/images/slidingwindow/3.png" alt="img" style="zoom: 33%;" />

直到窗口中的字符串不再符合要求，`left` 不再继续移动。

<img src="https://labuladong.gitee.io/algo/images/slidingwindow/4.png" alt="img" style="zoom: 33%;" />
...
<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/1.jpg" alt="img" style="zoom: 33%;" />
...
<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/2.jpg" alt="img" style="zoom: 33%;" />

<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/3.jpg" alt="img" style="zoom: 33%;" />

<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/4.jpg" alt="img" style="zoom: 33%;" />

<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/5.jpg" alt="img" style="zoom: 33%;" />
...
<img src="https://labuladong.gitee.io/algo/images/%E5%8F%8D%E8%BD%AC%E9%93%BE%E8%A1%A8/6.jpg" alt="img" style="zoom: 33%;" />
...
<img src="https://labuladong.gitee.io/algo/images/kgroup/1.jpg" alt="img" style="zoom: 33%;" />

如果我设法把前 2 个节点反转，那么后面的那些节点怎么处理？后面的这些节点也是一条链表，而且规模（长度）比原来这条链表小，这就叫子问题。

<img src="https://labuladong.gitee.io/algo/images/kgroup/5.jpg" alt="img" style="zoom: 33%;" />
...
力扣第 146 题「LRU缓存机制」，设计数据结构：首先要接收一个 `capacity` 参数作为缓存的最大容量，然后实现两个 API，一个是 `put(key, val)` 方法存入键值对，另一个是 `get(key)` 方法获取 `key` 对应的 `val`，如果 `key` 不存在则返回 -1。
...
<img src="https://labuladong.gitee.io/algo/images/LRU%E7%AE%97%E6%B3%95/4.jpg" alt="img" style="zoom: 50%;" />
...
<img src="https://labuladong.gitee.io/algo/images/LRU%e7%ae%97%e6%b3%95/put.jpg" alt="img" style="zoom: 33%;" />
...
LFU 算法的淘汰策略是 Least Frequently Used，也就是每次淘汰那些使用次数最少的数据，如果访问频次最低的数据有多条，需要淘汰最旧的数据。

写一个类，接受一个`capacity`参数，实现`get`和`put`方法：
...
3. 如果在容量满了的时候进行插入，则需要将`freq`最小的`key`删除，如果最小的`freq`对应多个`key`，则删除其中最旧的那一个。
    ...
    - 希望`freq`对应的`key`的列表是**存在时序**的，便于快速查找并删除最旧的`key`。
    - 希望**能够快速删除`key`列表中的任何一个`key`**，因为如果频次为`freq`的某个`key`被访问，那么它的频次就会变成`freq+1`，就应该从`freq`对应的`key`列表中删除，加到`freq+1`对应的`key`的列表中。

<img src="https://mmbiz.qpic.cn/sz_mmbiz_jpg/gibkIz0MVqdF5WfuuetibLgFV3FhuzwMaKpxO8BpoNOYKh7ZelxDpoJCiahWZV4DIuKESoslqicgic72icgm8mh122jA/640?wx_fmt=jpeg&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1" alt="图片" style="zoom: 33%;" />

```java
class LFUCache {
    HashMap<Integer, Integer> keyToVal; // key 到 val 的映射，我们后文称为 KV 表
    HashMap<Integer, Integer> keyToFreq; // key 到 freq 的映射，我们后文称为 KF 表
    HashMap<Integer, LinkedHashSet<Integer>> freqToKeys; // freq 到 key 列表的映射，我们后文称为 FK 表
    int minFreq; // 记录最小的频次
    int cap; // 记录 LFU 缓存的最大容量
    ...
    public void put(int key, int val) {
        ...
        /* key 不存在，需要插入。容量已满的话需要淘汰一个 freq 最小的 key */
        if (this.cap <= keyToVal.size()) {
            removeMinFreqKey();
        }

        /* 插入 key 和 val，对应的 freq 为 1 */
        keyToVal.put(key, val); // 插入 KV 表
        keyToFreq.put(key, 1); // 插入 KF 表
        freqToKeys.putIfAbsent(1, new LinkedHashSet<>()); // 插入 FK 表
    ...
    private void increaseFreq(int key) {
        ...
        /* 更新 FK 表 */
        freqToKeys.get(freq).remove(key); // 将 key 从 freq 对应的列表中删除
        freqToKeys.putIfAbsent(freq + 1, new LinkedHashSet<>()); // 将 key 加入 freq + 1 对应的列表中
        freqToKeys.get(freq + 1).add(key);
        if (freqToKeys.get(freq).isEmpty()) { // 如果 freq 对应的列表空了，移除这个 freq
...
```java
class FreqStack {
    int maxFreq = 0; // 记录 FreqStack 中元素的最大频率
    HashMap<Integer, Integer> valToFreq = new HashMap<>(); // 记录 FreqStack 中每个 val 对应的出现频率，称为 VF 表
    HashMap<Integer, Stack<Integer>> freqToVals = new HashMap<>(); // 记录频率 freq 对应的 val 列表，称为 FV 表
    
    public void push(int val) {
        int freq = valToFreq.getOrDefault(val, 0) + 1; // 修改 VF 表：val 对应的 freq 加一
        valToFreq.put(val, freq);
        freqToVals.putIfAbsent(freq, new Stack<>()); // 修改 FV 表：在 freq 对应的列表加上 val
        freqToVals.get(freq).push(val);
        maxFreq = Math.max(maxFreq, freq); // 更新 maxFreq
    }
    
    public int pop() {
        Stack<Integer> vals = freqToVals.get(maxFreq); // 修改 FV 表：pop 出一个 maxFreq 对应的元素 v
        int v = vals.pop();
        int freq = valToFreq.get(v) - 1; // 修改 VF 表：v 对应的 freq 减一
        valToFreq.put(v, freq);
        if (vals.isEmpty()) { // 更新 maxFreq
            maxFreq--;
        }
        return v;
    }
...
<img src="https://labuladong.gitee.io/algo/images/heap/1.png" alt="img" style="zoom: 20%;" />

优先级队列这种数据结构有一个很有用的特性，**插入或者删除元素的时候，元素会自动排序**，底层的原理就是二叉堆的操作。
...
public class MaxPQ
    ...
    public MaxPQ(int cap) {
        pq = (Key[]) new Comparable[cap + 1]; // 索引 0 不用，所以多分配一个空间
    }
    ...
    /* 删除并返回当前队列中最大元素 */
    public Key delMax() {
        Key max = pq[1]; // 最大堆的堆顶就是最大元素
        exch(1, N); // 把这个最大元素换到最后，删除之
        pq[N] = null;
        N--;
        sink(1); // 让 pq[1] 下沉到正确位置
        return max;
    }

    /* 上浮第 k 个元素，以维护最大堆性质 */
    private void swim(int k) {
        while (k > 1 && less(parent(k), k)) { // 如果浮到堆顶，就不能再上浮了
            exch(parent(k), k); // 如果第 k 个元素比上层大，将 k 换上去
            k = parent(k);
        }
    }
    ...
    private void sink(int k) {
        while (left(k) <= N) { // 如果沉到堆底，就沉不下去了
            int older = left(k); // 先假设左边节点较大
            if (right(k) <= N && less(older, right(k))) // 如果右边节点存在，比一下大小
                older = right(k);
            if (less(older, k)) // 结点 k 比俩孩子都大，就不必下沉了
                break;
            exch(k, older); // 否则，不符合最大堆的结构，下沉 k 结点
            k = older;
        }
    }
...
<img src="https://labuladong.gitee.io/algo/images/knapsack/1.png" alt="img" style="zoom: 50%;" />

1. 明确两点「状态」和「选择」。状态有两个，就是「背包的容量」和「可选择的物品」，选择就是「装进背包」或者「不装进背包」。

2. 明确 `dp` 数组的定义。「状态」有两个，也就需要一个二维 `dp` 数组，`dp[i][w]` 的定义如下：对于前 `i` 个物品，当前背包的容量为 `w`，这种情况下可以装的最大价值是 `dp[i][w]`。

3. 根据「选择」，思考状态转移的逻辑。

    如果你没有把这第 `i` 个物品装入背包，那么最大价值 `dp[i][w]` 应该等于 `dp[i-1][w]`，继承之前的结果。
